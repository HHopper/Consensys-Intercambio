...
contract('Intercambio', function (accounts) {    
    ...
    it('should have a contract balance of undefined because the constructor is not payable', async () => {
     ...
     ///@dev this test makes sure the circuit breaker actually works. I wrote it becuase, well, if it doesn't work, the contract would be very vulnerable.
     it('when called by the owner the circuit breaker should stop all fund movement', async () => {
        ...
        assert.equal(result2, expected2), "the student can still add funds to their account even after the circuit breaker has been called";
    });
